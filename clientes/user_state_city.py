...
st.subheader(f"Top5 estados con más clientes desde: {fechaElegida[0]}  hasta: {fechaElegida[1]}",anchor=False)
...
x = df_city_state_group_u['customer_city'].head(5).str.capitalize().values
y = df_city_state_group_u['customer_unique_id'].head(5).values
c = df_city_state_group_u['customer_state'].head(5).apply(lambda x: state_to_color[x])

labels_in = [l for l in state_to_color.keys() if l in df_city_state_group_u['customer_state'].head(5).values]
